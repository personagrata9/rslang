...
class WordCard {
  ...
  private createWordElement = (): HTMLDivElement => {
    const wordElement: HTMLDivElement = createDivElement(`${this.className}-item mb-4 d-flex flex-wrap`);

    const word: HTMLHeadingElement = createHeadingElement(
      'h3',
      `${this.className}-word text-capitalize`,
      this.word.word
    );
  ...
  private createAudioIcon = (): HTMLElement => {
    const audioIcon = createDivElement(
      `${this.className}-audio-icon d-flex justify-content-center align-items-center shadow`
    );
    audioIcon.innerHTML =
